...
it('can perform the request', closure: function () {
    if (shouldResetFixtures()) {
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/fetch-a-list-of-reminders.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/create-an-invoice-for-list-reminders.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/issue-an-invoice-for-list-reminders.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/create-a-reminder-for-list.json');
    }

    Saloon::fake([
        CreateAnInvoiceRequest::class => MockResponse::fixture('Invoices/Reminders/create-an-invoice-for-list-reminders'),
        ...
        IssueAnInvoiceRequest::class => MockResponse::fixture('Invoices/Reminders/issue-an-invoice-for-list-reminders'),
        CreateAReminderRequest::class => MockResponse::fixture('Invoices/Reminders/create-a-reminder-for-list'),
        FetchAListOfRemindersRequest::class => MockResponse::fixture('Invoices/Reminders/fetch-a-list-of-reminders'),
    ]);

    $connector = new BexioConnector(new ConnectWithToken);

    $invoiceId = createReminderFixtureContext($connector);

    if ($invoiceId === null) {
        $this->markTestSkipped('Unable to obtain an issued invoice with a reminder.');
    }

    $response = $connector->send(new FetchAListOfRemindersRequest(invoice_id: $invoiceId));
...
function createReminderFixtureContext(BexioConnector $connector): ?int
{
    ...
    $connector->send(new CreateAReminderRequest(invoice_id: $invoice->id, reminder: $reminder));
