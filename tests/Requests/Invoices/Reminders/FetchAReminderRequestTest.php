...
it('can perform the request', closure: function () {
    if (shouldResetFixtures()) {
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/fetch-a-reminder.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/create-an-invoice-for-fetch-reminder.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/issue-an-invoice-for-fetch-reminder.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Reminders/create-a-reminder-for-fetch.json');
    }

    Saloon::fake([
        CreateAnInvoiceRequest::class => MockResponse::fixture('Invoices/Reminders/create-an-invoice-for-fetch-reminder'),
        ...
        IssueAnInvoiceRequest::class => MockResponse::fixture('Invoices/Reminders/issue-an-invoice-for-fetch-reminder'),
        CreateAReminderRequest::class => MockResponse::fixture('Invoices/Reminders/create-a-reminder-for-fetch'),
        FetchAReminderRequest::class => MockResponse::fixture('Invoices/Reminders/fetch-a-reminder'),
    ]);

    $connector = new BexioConnector(new ConnectWithToken);

    [$invoiceId, $reminderId] = createReminderForFetch($connector);

    if ($invoiceId === null || $reminderId === null) {
        $this->markTestSkipped('Unable to create an issued invoice with a reminder.');
    }

    $response = $connector->send(new FetchAReminderRequest(invoice_id: $invoiceId, reminder_id: $reminderId));
...
function createReminderForFetch(BexioConnector $connector): array
{
