...
it('can perform the request', closure: function () {
    if (shouldResetFixtures()) {
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/cancel-an-invoice.json');
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/create-an-invoice-for-cancel.json');
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/issue-an-invoice-for-cancel.json');
    }
    ...
    Saloon::fake([
        ...
        CreateAnInvoiceRequest::class => MockResponse::fixture('Invoices/create-an-invoice-for-cancel'),
        IssueAnInvoiceRequest::class => MockResponse::fixture('Invoices/issue-an-invoice-for-cancel'),
    ...
    // An invoice can only be cancelled once it has been issued.
    $connector->send(new IssueAnInvoiceRequest(invoice_id: $invoice->id));

    $response = $connector->send(new CancelAnInvoiceRequest(invoice_id: $invoice->id));

    if (! $response->successful()) {
        $this->markTestSkipped(sprintf(
            'Invoice cancellation rejected by API (HTTP %d): %s',
            $response->status(),
            $response->body(),
        ));
    }
