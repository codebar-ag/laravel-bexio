...
it('can perform the request', closure: function () {
    if (shouldResetFixtures()) {
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/copy-an-invoice.json');
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/create-an-invoice-for-copy.json');
    }
    ...
    Saloon::fake([
        ...
        CreateAnInvoiceRequest::class => MockResponse::fixture('Invoices/create-an-invoice-for-copy'),
    ...
    $contactId = $contacts->dto()->first()->id;
    ...
    if ($invoice === null) {
        $this->markTestSkipped('Unable to create an invoice to copy.');
    }

    $response = $connector->send(new CopyAnInvoiceRequest(invoice_id: $invoice->id, contact_id: $contactId));
