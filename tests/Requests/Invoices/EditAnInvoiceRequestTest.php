...
it('can perform the request', closure: function () {
    if (shouldResetFixtures()) {
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/edit-an-invoice.json');
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/fetch-an-invoice-edit.json');
        @unlink(__DIR__.'/../../Fixtures/Saloon/Invoices/create-an-invoice-for-edit.json');
    }

    Saloon::fake([
        CreateAnInvoiceRequest::class => MockResponse::fixture('Invoices/create-an-invoice-for-edit'),
    ...
    // Editing is only allowed on draft invoices, so create a fresh one inline.
    ...
    $created = $connector->send(new CreateAnInvoiceRequest(invoice: $newInvoice))->dto();

    if ($created === null) {
        $this->markTestSkipped('Unable to create a draft invoice to edit.');
    }

    $invoice = $connector->send(new FetchAnInvoiceRequest(invoice_id: $created->id))->dto();

    expect($invoice)->toBeInstanceOf(InvoiceDTO::class);

    $invoice->title = 'Test Invoice';

    $response = $connector->send(new EditAnInvoiceRequest(invoice_id: $created->id, invoice: $invoice));
