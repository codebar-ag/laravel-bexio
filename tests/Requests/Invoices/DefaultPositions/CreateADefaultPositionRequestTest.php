...
it('can perform the request', closure: function () {
    if (shouldResetFixtures()) {
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/DefaultPositions/create-a-default-position.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/DefaultPositions/create-an-invoice-for-default-position.json');
    }

    Saloon::fake([
        CreateAnInvoiceRequest::class => MockResponse::fixture('Invoices/DefaultPositions/create-an-invoice-for-default-position'),
    ...
    $accountId = $accounts->dto()->filter(fn ($account) => $account->account_type === AccountTypeEnum::EARNINGS()->value)->first()->id;

    // Positions can only be added to a draft document, so create one inline.
    ...
    if ($invoice === null) {
        $this->markTestSkipped('Unable to create a draft invoice to add a position to.');
    }
