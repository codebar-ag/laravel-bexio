...
it('can perform the request', closure: function () {
    if (shouldResetFixtures()) {
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Payments/create-a-payment.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Payments/create-an-invoice-for-payment.json');
        @unlink(__DIR__.'/../../../Fixtures/Saloon/Invoices/Payments/issue-an-invoice-for-payment.json');
    }

    Saloon::fake([
        CreateAnInvoiceRequest::class => MockResponse::fixture('Invoices/Payments/create-an-invoice-for-payment'),
        ...
        IssueAnInvoiceRequest::class => MockResponse::fixture('Invoices/Payments/issue-an-invoice-for-payment'),
        CreateAPaymentRequest::class => MockResponse::fixture('Invoices/Payments/create-a-payment'),
    ]);

    $connector = new BexioConnector(new ConnectWithToken);

    [$invoice, $paymentAccountId] = createIssuedInvoiceForPayment($connector);

    if ($invoice === null) {
        $this->markTestSkipped('Unable to create and issue an invoice to add a payment to.');
    }
    ...
    $response = $connector->send(new CreateAPaymentRequest(invoice_id: $invoice->id, payment: $payment));
...
function createIssuedInvoiceForPayment(BexioConnector $connector): array
{
