...
use CodebarAg\Bexio\Requests\Countries\FetchACountryRequest;
use CodebarAg\Bexio\Requests\Countries\FetchAListOfCountriesRequest;
use Saloon\Http\Faking\MockResponse;
use Saloon\Laravel\Saloon;

it('can perform the request', closure: function () {
    $fixturePath = __DIR__.'/../../Fixtures/Saloon/Countries/fetch-a-country.json';
    ...
    Saloon::fake([
        FetchACountryRequest::class => MockResponse::fixture('Countries/fetch-a-country'),
    ...
    if (! $existingCountry) {
        $this->markTestSkipped('No countries found in the system to fetch');
    }

    $response = $connector->send(new FetchACountryRequest(country_id: $existingCountry->id));

    expect($response->successful())->toBeTrue();
    expect($response->dto())->toBeInstanceOf(CountryDTO::class);

    Saloon::assertSent(FetchACountryRequest::class);
})->group('countries');
